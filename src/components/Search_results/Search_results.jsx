...
function Search_results({ match }) {
    ...
    if (results.searchType == "care") {
      db.collection("hospitals").onSnapshot((snap) => {
        let data = snap.docs.reduce((acc, doc) => {
          if (
            doc.data().address.governorate == `${results.city}` &&
            doc.data().hasOwnProperty("intensiveCares") === true &&
            doc.data().intensiveCares > 0
          ) {
    ...
    // search for hospitals
      ...
      if (
        ...
        db.collection("hospitals")
          .where("address.governorate", "==", `${results.city}`)
          .get()
          .then((querySnapshot) => {
        ...
        db.collection("hospitals")
          ...
          .then((querySnapshot) => {
            let searchResults = querySnapshot.docs.map((doc) => {
              // doc.data() is never undefined for query doc snapshots
              if (
                doc.data().specialities.hasOwnProperty(`${results.special}`) ===
                true
              ) {
                return { ...doc.data(), id: doc.id };
              }
          ...
          .then((querySnapshot) => {
            let searchResults = querySnapshot.docs.reduce((acc, doc) => {
              ...
              console.log(
                doc.data().specialities.hasOwnProperty(`${results.special}`) ==
                  true &&
                  (doc.data().address.governorate == `${results.city}`) == true
              );
              if (
                doc.data().specialities.hasOwnProperty(`${results.special}`) ==
                  true &&
        ...
        .then((querySnapshot) => {
          let searchResults = querySnapshot.docs.reduce((acc, doc) => {
            ...
            if (
              ...
              results.name == "none" &&
              results.special !== "none" &&
              doc.data().specialities[results.special]
            ) {
              console.log(doc.id);
              return [
                ...acc,
                {
                  results: [...doc.data().specialities[results.special]],
                  hospitalTitle: doc.data().name,
                  hospitalCity: doc.data().address.governorate,
                  hospitalId: doc.id,
                  speciality: `${results.special}`,
                },
              ];
            }
  ...
  const currentHospitalResults = hospitals.slice(
    indexOfFirstResult,
    indexOfLastResult
  );
  const currentDoctorResults = doctors.slice(
    indexOfFirstResult,
    indexOfLastResult
  );
  return (
